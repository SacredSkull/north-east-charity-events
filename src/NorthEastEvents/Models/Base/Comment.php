...
abstract class Comment implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\NorthEastEvents\\Models\\Map\\CommentTableMap';
    ...
    /**
     * Initializes internal state of NorthEastEvents\Models\Base\Comment object.
     */
     ...
     * @return $this|\NorthEastEvents\Models\Comment The current object (for fluent API support)
     */
    ...
    public function setBodyHTML($v)
    {
        ...
        if ($this->bodyhtml !== $v) {
            $this->bodyhtml = $v;
            $this->modifiedColumns[CommentTableMap::COL_BODYHTML] = true;
        }
    ...
    public function setThreadID($v)
    {
        ...
        if ($this->threadid !== $v) {
            $this->threadid = $v;
            $this->modifiedColumns[CommentTableMap::COL_THREADID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CommentTableMap::translateFieldName('BodyHTML', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : CommentTableMap::translateFieldName('ThreadID', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\NorthEastEvents\\Models\\Comment'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_BODYHTML)) {
            $modifiedColumns[':p' . $index++]  = 'bodyHTML';
        }
        if ($this->isColumnModified(CommentTableMap::COL_USERID)) {
            $modifiedColumns[':p' . $index++]  = 'userID';
        }
        if ($this->isColumnModified(CommentTableMap::COL_THREADID)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\NorthEastEvents\Models\Comment
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\NorthEastEvents\Models\Comment The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_BODYHTML)) {
            $criteria->add(CommentTableMap::COL_BODYHTML, $this->bodyhtml);
        }
        if ($this->isColumnModified(CommentTableMap::COL_USERID)) {
            $criteria->add(CommentTableMap::COL_USERID, $this->userid);
        }
        if ($this->isColumnModified(CommentTableMap::COL_THREADID)) {
            $criteria->add(CommentTableMap::COL_THREADID, $this->threadid);
        }
        if ($this->isColumnModified(CommentTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \NorthEastEvents\Models\Comment (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \NorthEastEvents\Models\Comment Clone of current object.
     * @throws PropelException
     */
